 ...
 *
 * Configures Dynamoose with LocalStack for local development
 * or AWS for production
 */
export function initializeDynamoDB(): void {
  ...
  try {
    if (Environment.IS_LOCAL && Environment.DYNAMODB_ENDPOINT) {
      ...
      // Use default AWS SDK configuration
      // Credentials will be picked up from Lambda execution role
    }
